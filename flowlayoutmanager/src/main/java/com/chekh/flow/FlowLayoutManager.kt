...
import com.chekh.flow.adapter.FlowListAdapter
...
class FlowLayoutManager @JvmOverloads constructor(
    ...
    private fun updateLayout() {
        ...
        (0 until adapter.itemCount).forEach { position ->
            rectList.add(getItemBounds(position, adapter))
        }
    }

    private fun getItemBounds(position: Int, adapter: FlowListAdapter<*>): Rect {
        val x = adapter.getItemX(position, columnWidth)
        val y = adapter.getItemY(position, rowHeight)
        val width = adapter.getItemWidth(position, columnWidth)
        val height = adapter.getItemHeight(position, rowHeight)
    ...
    private fun Rect.isIntersect(other: Rect) =
